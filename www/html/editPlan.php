...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    #Vypis
    #echo "<pre>".print_r($_POST["workouts"],true)."</pre>";
        ...
        <div id="workouts" class="space-y-4">
            ...
            foreach ($plan->workout as $workout) { 
                ...
                <div class="workout bg-gray-100 p-4 rounded-lg shadow">
                    ...
                    <input step="0.01" type="number" name="workouts[<?php echo $index; ?>][trvani]" value="<?php echo htmlspecialchars($workout->trvani); ?>" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                    <div class="cviky space-y-2">
                        <?php foreach ($workout->cviky->cvik as $cvikIndex => $cvik) { ?>
                            <div class="cvik bg-white p-2 rounded-lg shadow mt-2">
                                ...
                                <input step="0.01" type="number" name="workouts[<?php echo $index; ?>][cviky][<?php echo $cvikIndex; ?>][odpocinek]" value="<?php echo htmlspecialchars($cvik->odpocinek); ?>" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            </div>
                        <?php } ?>
                    </div>
...
<script>
    ...
    function addExercise(workoutDiv, workoutCount) {
        ...
        exerciseDiv.innerHTML = `
            ...
            <input type="number" name="workouts[${workoutCount}][cviky][${exerciseCount}][odpocinek]" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        `;
    ...
    function addWorkout() {
        ...
        workoutDiv.innerHTML = `
            ...
            <label class="block text-sm font-medium text-gray-700">Trvání (min.):</label>
            <input step="0.01" type="number" name="workouts[${workoutCount}][trvani]" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
